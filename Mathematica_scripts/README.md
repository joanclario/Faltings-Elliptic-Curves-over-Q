...
In particular, in the case of the type $L_3(9)$, extensive explanatory text is included throughout the notebook `L_3(9).nb` in order to clarify the intermediate steps and to make the procedure reproducible. The remaining cases can be followed in a similar way, using the `L3(9)` notebook as a detailed guide.
